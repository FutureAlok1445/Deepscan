    ...
    try:
        ...
        if mime_type.startswith("image/"):
            ...
            result = {
                ...
                "confidence": 0.95,
                "sub_scores": {
                    "MAS": signals.get("MAS", score),
                    "PPS": signals.get("PPS", None),
                    "IRS": signals.get("IRS", None),
                    "AAS": None,
                    "CVS": signals.get("CVS", None),
                },
                "explanation": _generate_explanation(verdict, ["IMageDetector-10Layer"], signals),
                "gradcam_available": False,
                "gradcam_base64": None,
                "detectors_used": list(signals.keys()) if signals else ["IMageDetector"],
                "processing_time_ms": elapsed,
                "fallback_used": False,
                "file_type": mime_type,
                "original_filename": file.filename,
                "findings": [
                    {"engine": k, "score": v, "detail": f"{k} analyzed this layer"}
